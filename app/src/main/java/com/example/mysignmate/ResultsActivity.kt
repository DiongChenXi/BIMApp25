...
import com.example.mysignmate.utils.VideoProcessorUtil
...
class ResultsActivity : AppCompatActivity() {
    ...
    private lateinit var videoProcessor: VideoProcessorUtil
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        videoProcessor = VideoProcessorUtil(applicationContext)
    ...
    private fun processVideo(uri: android.net.Uri) {
        // Crucial: Perform video processing on a background thread
        lifecycleScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            Log.d(TAG, "Starting video processing for URI: $uri")
            ...
            try {
                retriever.setDataSource(applicationContext, uri) // Use applicationContext for broader scope if needed
                ...
                if (videoDurationMs > 0) {
                    ...
                    while (currentTimeMs < videoDurationMs) {
                        ...
                        if (bitmapFrame != null) {
                            ...
                            Log.w(TAG, "Could not retrieve frame at $currentTimeMs ms for URI: $uri")
                        }
                    ...
                    handleProcessedLandmarks(allFrameHandLandmarks, allFramePoseLandmarks)

                } else {
                    Log.e(TAG, "Video duration is 0 or could not be determined for URI: $uri")
//                    kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
//                        processingListener?.onError("Could not determine video duration for URI: $uri")
//                    }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error during video processing for URI: $uri", e)
//                kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
...
//                }
    ...
    private fun handleProcessedLandmarks(
        handLandmarkResults: List<HandLandmarkerResult?>,
        poseLandmarkResults: List<PoseLandmarkerResult?>
    ) {
        lifecycleScope.launch(kotlinx.coroutines.Dispatchers.Main) {
            Toast.makeText(this@ResultsActivity, "All landmarks collected. Hand results: ${handLandmarkResults.size}, Pose results: ${poseLandmarkResults.size}", Toast.LENGTH_LONG).show()
            ...
            if (handLandmarkResults.size != poseLandmarkResults.size) {
                Toast.makeText(this@ResultsActivity, "Warning: Hand and Pose landmark result counts differ. Processing up to the minimum count: $numFrames", Toast.LENGTH_LONG).show()
            }
            ...
            if (allFramesKeypoints.isNotEmpty()) {
                ...
                val numberOfFrames = finalKeypoints.size
                val keypointsPerFrame = if (numberOfFrames > 0) finalKeypoints[0].size else 0
                resultsTextView.text = "Landmark processing complete for hands and pose! Processed keypoints shape: ($numberOfFrames, $keypointsPerFrame)"
