 ...
 */
//https://github.com/google-ai-edge/mediapipe-samples/blob/main/examples/pose_landmarker/android/app/src/main/java/com/google/mediapipe/examples/poselandmarker/PoseLandmarkerHelper.kt
...
//        imageProxy.close()
    ...
    fun detectAsync(mpImage: MPImage, frameTime: Long) {
        ...
        // As we're using running mode LIVE_STREAM, the landmark result will be returned in returnLivestreamResult function
    }

    // Accepts the URI for a video file loaded from the user's gallery and attempts to run pose landmarker inference on the video. This process will evaluate every frame in the video and attach the results to a bundle that will be returned.
    fun detectVideoFile(
        videoUri: Uri,
        ...
        // Note: We need to read width/height from frame instead of getting the width/height of the video directly because MediaRetriever returns frames that are smaller than the actual dimension of the video file.
        ...
        return if (didErrorOccurred) {
            ...
            PoseResultBundle(resultList, inferenceTimePerFrameMs, height, width)
        }
    }

    // Accepted a Bitmap and runs pose landmarker inference on it to return
    // results back to the caller
    fun detectImage(image: Bitmap): PoseResultBundle? {
        ...
        // If poseLandmarker?.detect() returns null, this is likely an error. Returning null to indicate this.
        poseLandmarkerHelperListener?.onPoseError(
